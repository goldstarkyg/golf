...
class VIPFetchList extends AbstractInterface
{
    ...
    public function verifyInput(&$args)
    {
     //   $req = $args['interface']['para'];
        ...
        $rules = array(
			'vip_userid' => array('type' => 'string'),
            'flag' => array('type' => 'int', 'range' => '[0,+)'),
        	'pageno'=>array('type' => 'int', 'range' => '[0,+)'),
			'cat_id' => array('type' => 'int', 'range' => '[0,+)'),
    ...
    public function process()
    {
        ...
        elseif ($this->_args['flag'] == GET_LIST_TYPE_ALL)       
        {
        	...
        	$ret = $dao_live->getLiveCount($live_count, $error_message, $cat_id);
        	if($ret == 0)
        	{
				$vip_userid = '';
				if(array_key_exists('vip_userid', $this->_args))
				{
					$vip_userid = $this->_args['vip_userid'];
				}
				$ret = $dao_live->getUGCCount($ugc_count, $error_message, $vip_userid, $cat_id);


				if($ret == 0)
        		{        			
        			interface_log(DEBUG, EC_OK, 'FetchList all count:' .strval($live_count) . ":" . strval($ugc_count). "startpos:". strval($start_pos) . "num:" .strval($page_size));
        			$all_count = $live_count + $ugc_count;
	        		if($live_count >= ($start_pos+$page_size))
	        		{
	        			$ret = $dao_live->getVIPDataList(GET_LIST_TYPE_ONLINE,$start_pos, $page_size,$result_list, $error_message, $cat_id, $vip_userid);
	        		}        		
	        		elseif($live_count <= $start_pos)
	        		{           			
						$start_pos -=  $live_count ;      		
	        		    //$ret = $dao_live->getDataList(GET_LIST_TYPE_TAPE,$start_pos, $page_size,$result_list, $error_message);
						$ret = $dao_live->getUGCList( $start_pos, $page_size,$result_list, $error_message, $vip_userid, $cat_id);
	        		...
	        		else 
	        		{
	        			...
	        			$ret = $dao_live->getVIPDataList(GET_LIST_TYPE_ONLINE,$start_pos, $tmp_live_num, $live_list, $error_message, $cat_id, $vip_userid);

	        			if($ret ==0)
	        			{		        				
	        				$rest_num = $page_size - $tmp_live_num;
							$ret = $dao_live->getUGCList( 0, $rest_num,$ugc_list, $error_message, $vip_userid, $cat_id);
        ...
        elseif($this->_args['flag'] ==GET_LIST_UGC_DATA)
        {
        	...
        	$ret = $dao_live->getUGCCount($all_count, $error_message, $user_id);
        	if($ret == 0)
        	{
        		$ret = $dao_live->getUGCList( $start_pos, $page_size,$result_list, $error_message, $user_id);
        	}       	
        }
