...
if($do == 'list') {
    $cid = intval($_GPC['cid']);
    $category = pdo_fetch("SELECT * FROM ".tablename('article_category')." WHERE id = '{$cid}' AND uniacid = '{$_W['uniacid']}'");
    if (empty($category)) {
    ...
    if (empty($category['ishomepage'])) {
        ...
        $ishomepage = 1;
        $navs = pdo_fetchall("SELECT * FROM ".tablename('article_category')." WHERE uniacid = '{$_W['uniacid']}' AND parentid = '$cid' ORDER BY displayorder DESC,id DESC");
        if (!empty($navs)) {
            foreach ($navs as &$row) {
    ...
    $id = intval($_GPC['id']);
    $sql = "SELECT * FROM " . tablename('golf_article') . " WHERE `id`=:id AND uniacid = :uniacid";
    ...
    pdo_update('golf_article', array('click' => $detail['click']), array('uniacid' => $_W['uniacid'], 'id' => $id));
    $_share = array('desc' => $detail['description'], 'title' => $detail['title'], 'imgUrl' => $detail['thumb']);
    template('site/detail');
} elseif($do == 'handsel') {
    if($_W['ispost']) {
        $id = intval($_GPC['id']);
        $article = pdo_fetch('SELECT id, credit FROM ' . tablename('golf_article') . ' WHERE uniacid = :uniacid AND id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
