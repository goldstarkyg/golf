...
class DB {
	...
	public function update_table($table, $data = array(), $params = array(), $glue = 'AND') {
		$fields = $this->implode($data, ',');
		$condition = $this->implode($params, $glue);
		$params = array_merge($fields['params'], $condition['params']);
	...
	public function insert_table($table, $data = array(), $replace = FALSE) {
		$cmd = $replace ? 'REPLACE INTO' : 'INSERT INTO';
		$condition = $this->implode($data, ',');
	...
	public function delete_table($table, $params = array(), $glue = 'AND') {
		$condition = $this->implode($params, $glue);
