...
class Temp_EweiShopV2Page extends ComWebPage
{
    ...
    public function main()
    {
        ...
        $items = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE uniacid=:uniacid and merchid=0 and title like :name order by id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize,
            array(
                ':name' => '%' . $kw . '%',
                ':uniacid' => $_W['uniacid']
            )
        );
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE uniacid=:uniacid and merchid=0 and title like :name order by id desc ',
            array(
                ':uniacid' => $_W['uniacid'],
                ':name' => '%' . $kw . '%'
            )
