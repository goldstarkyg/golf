...
class Goods_EweiShopV2Model
{
    ...
    {
        global $_W;
        return pdo_fetch('select * from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid Limit 1',
            array(
                ':id' => $optionid,
                ':uniacid' => $_W['uniacid'],
                ':goodsid' => $goodsid
    ...
    {
        global $_W;
        return pdo_fetchcolumn('select marketprice from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid',
            array(
                ':id' => $optionid,
                ':uniacid' => $_W['uniacid'],
                ':goodsid' => $goodsid
    ...
    public function getTotals()
    {
        global $_W;
        return array(
            'sale' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where status=1 and checked=0 and deleted=0 and total>0 and uniacid=:uniacid',
                array(':uniacid' => $_W['uniacid'])
            ),
            'out' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where status=1 and deleted=0 and total=0 and uniacid=:uniacid',
                array(':uniacid' => $_W['uniacid'])
            ),
            'stock' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid',
                array(':uniacid' => $_W['uniacid'])
            ),
            'cycle' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where deleted=1 and uniacid=:uniacid',
                array(':uniacid' => $_W['uniacid'])
            )
        );
    }
    ...
    public function isFavorite($id = '')
    {
        ...
        $count = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member_favorite') . ' where goodsid=:goodsid and deleted=0 and openid=:openid and uniacid=:uniacid limit 1',
            array(
                ':goodsid' => $id,
                ':openid' => $_W['openid'],
                ':uniacid' => $_W['uniacid']
    ...
    public function addHistory($goodsid = 0)
    {
        ...
        pdo_query('update ' . tablename('ewei_shop_goods') . ' set viewcount=viewcount+1 where id=:id and uniacid=\'' . $_W[uniacid] . '\' ',
            array(':id' => $goodsid)
        );
    ...
    public function getCartCount()
    {
        global $_W;
        global $_GPC;
        $count = pdo_fetchcolumn('select sum(total) from ' . tablename('ewei_shop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 limit 1',
            array(
                ':openid' => $_W['openid'],
                ':uniacid' => $_W['uniacid']
            )
    ...
    public function getSpecThumb($specs)
    {
        ...
        if (!empty($specid)) {
            $spec = pdo_fetch('select thumb from ' . tablename('ewei_shop_goods_spec_item') . ' ' . ' where id=:id and uniacid=:uniacid limit 1 ',
                array(
                    ':id' => $specid,
                    ':uniacid' => $_W['uniacid']
                )
    ...
    public function canBuyAgain($goods)
    {
        ...
        $order_goods = pdo_fetchall(
            'SELECT id,orderid FROM ' . tablename('ewei_shop_order_goods') . ' WHERE uniacid=:uniaicd AND openid=:openid AND goodsid=:goodsid ' . $condition,
            array(
                ':uniaicd' => $_W['uniacid'],
                ':openid' => $_W['openid'],
                'goodsid' => $id
        ...
        $order = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE uniacid=:uniaicd AND status>=:status AND id IN (' . implode(',', array_keys($order_goods)) . ')',
            array(
                ':uniaicd' => $_W['uniacid'],
    ...
    {
        global $_W;
        $order_goods = pdo_fetchall('SELECT id,goodsid FROM ' . tablename('ewei_shop_order_goods') . ' WHERE uniacid=:uniaicd AND openid=:openid AND canbuyagain = 1 AND orderid <> :orderid',
            array(':uniaicd' => $_W['uniacid'], ':openid' => $_W['openid'], 'orderid' => $orderid), 'goodsid');
        if (empty($order_goods)) {
            return false;
        }
