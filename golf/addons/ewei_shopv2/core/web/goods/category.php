        ...
        if ($_W['ispost']) {
            ...
            if (!empty($_GPC['datas'])) {
                ...
                foreach ($datas as $row) {
                    $cateids[] = $row['id'];
                    pdo_update('ewei_shop_category', array('parentid' => 0, 'displayorder' => $displayorder, 'level' => 1),
                        array('id' => $row['id'])
                    );
                    if ($row['children'] && is_array($row['children'])) {
                        ...
                        foreach ($row['children'] as $child) {
                            $cateids[] = $child['id'];
                            pdo_query('update ' . tablename('ewei_shop_category') . ' set  parentid=:parentid,displayorder=:displayorder,level=2 where id=:id',
                                array(
                            ...
                            if ($child['children'] && is_array($child['children'])) {
                                ...
                                foreach ($child['children'] as $third) {
                                    $cateids[] = $third['id'];
                                    pdo_query('update ' . tablename('ewei_shop_category') . ' set  parentid=:parentid,displayorder=:displayorder,level=3 where id=:id',
                                        array(
