...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			load()->model('account');
			$data = array('uniacid' => $_W['uniacid'], 'days' => intval($_GPC['days']) * 24 * 3600, 'title' => trim($_GPC['title']), 'keyword' => trim($_GPC['keyword']), 'bg' => save_media($_GPC['bg']), 'data' => htmlspecialchars_decode($_GPC['data']), 'resptype' => trim($_GPC['resptype']), 'resptext' => trim($_GPC['resptext']), 'resptitle' => trim($_GPC['resptitle']), 'respthumb' => trim($_GPC['respthumb']), 'respdesc' => trim($_GPC['respdesc']), 'respurl' => trim($_GPC['respurl']), 'createtime' => time(), 'oktext' => trim($_GPC['oktext']), 'waittext' => trim($_GPC['waittext']), 'bedown' => intval($_GPC['bedown']), 'beagent' => intval($_GPC['beagent']), 'getposter' => trim($_GPC['getposter']), 'timestart' => strtotime($_GPC['time']['start']), 'timeend' => strtotime($_GPC['time']['end']), 'is_repeat' => intval($_GPC['is_repeat']), 'status' => intval($_GPC['status']), 'starttext' => trim($_GPC['starttext']), 'endtext' => trim($_GPC['endtext']), 'needcount' => intval($_GPC['needcount']));
			$reward = array();
	...
	public function delreward() 
	{
		...
		else if ($type == 2) 
		{
			...
			$item['reward_data'] = serialize($item['reward_data']);
			pdo_update(tablename('ewei_shop_task_poster'), array('reward_data' => $item['reward_data']), array('id' => $id, 'uniacid' => $_W['uniacid']));
			$item['reward_data'] = unserialize($item['reward_data']);
