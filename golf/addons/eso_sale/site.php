...
//                }
    ...
    public function doWebGoodProperty1()
    {
        global $_W, $_GPC;
        ...
        if ($operation == 'display') {
            if (!empty($_GPC['displayorder'])) {
                foreach ($_GPC['displayorder'] as $id => $displayorder) {
                    pdo_update('eso_sale_goods_property1', array('displayorder' => $displayorder), array('id' => $id));
                }
                message('分类排序更新成功！', $this->createWebUrl('goodproperty1', array('op' => 'display')), 'success');
            }
            $property = pdo_fetchall("SELECT * FROM " . tablename('eso_sale_goods_property1') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");

            include $this->template('goodproperty1');
        } elseif ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $property = pdo_fetch("SELECT * FROM " . tablename('eso_sale_goods_property1') . " WHERE id = '$id'");
            } else {
                $property = array(
                    'displayorder' => 0,
                );
            ...
            if (checksubmit('submit')) {
                if (empty($_GPC['propertyname'])) {
                    message('抱歉，请输入分类名称！');
                }
                $data = array(
                    'uniacid' => $_W['uniacid'],
                    'name' => $_GPC['propertyname'],
                    'enabled' => intval($_GPC['enabled']),
                    'displayorder' => intval($_GPC['displayorder']),
                    'isrecommand' => intval($_GPC['isrecommand']),
                    'ishot' => intval($_GPC['ishot']),
                ...
                if (!empty($id)) {
                    pdo_update('eso_sale_goods_property1', $data, array('id' => $id));
                } else {
                    pdo_insert('eso_sale_goods_property1', $data);
                    $id = pdo_insertid();
                }
                message('更新分类成功！', $this->createWebUrl('goodproperty1', array('op' => 'display')), 'success');
            }
            include $this->template('goodproperty1');
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $property = pdo_fetch("SELECT id FROM " . tablename('eso_sale_goods_property1') . " WHERE id = '$id'");
            if (empty($property)) {
                message('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('goodproperty1', array('op' => 'display')), 'error');
            }
            pdo_delete('eso_sale_goods_property1', array('id' => $id));
            message('分类删除成功！', $this->createWebUrl('goodproperty1', array('op' => 'display')), 'success');
        }
    }

    public function doWebGoodProperty2()
    {
        global $_W, $_GPC;
        ...
        if ($operation == 'display') {
            if (!empty($_GPC['displayorder'])) {
                foreach ($_GPC['displayorder'] as $id => $displayorder) {
                    pdo_update('eso_sale_goods_property2', array('displayorder' => $displayorder), array('id' => $id));
                }
                message('分类排序更新成功！', $this->createWebUrl('goodproperty2', array('op' => 'display')), 'success');
            }
            $property = pdo_fetchall("SELECT * FROM " . tablename('eso_sale_goods_property2') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");

            include $this->template('goodproperty2');
        } elseif ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $property = pdo_fetch("SELECT * FROM " . tablename('eso_sale_goods_property2') . " WHERE id = '$id'");
            } else {
                $property = array(
                    'displayorder' => 0,
                );
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    pdo_update('eso_sale_goods_property2', $data, array('id' => $id));
                } else {
                    pdo_insert('eso_sale_goods_property2', $data);
                    $id = pdo_insertid();
                }
                message('更新分类成功！', $this->createWebUrl('goodproperty2', array('op' => 'display')), 'success');
            }
            include $this->template('goodproperty2');
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $property = pdo_fetch("SELECT id FROM " . tablename('eso_sale_goods_property2') . " WHERE id = '$id'");
            if (empty($property)) {
                message('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('goodproperty2', array('op' => 'display')), 'error');
            }
            pdo_delete('eso_sale_goods_property2', array('id' => $id));
            message('分类删除成功！', $this->createWebUrl('goodproperty2', array('op' => 'display')), 'success');
        }
    }
    ...
    public function doWebSetGoodsProperty()
    {
        ...
        if (!in_array($type, array('new', 'hot', 'recommand', 'discount', 'status','adv'))) {
    ...
    public function doWebGoods()
    {
        ...
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        if ($operation == 'post') {
            $property_attr = $_GPC['property_attr'];
            $goods_id = intval($_GPC['id']);
            if($property_attr == 'add'){
                $data = array(
                    'uniacid'   => intval($_W['uniacid']),
                    'property1' => intval($_GPC['property1_list']),
                    'property2' => $_GPC['property2_list'],
                    'stock'     => intval($_GPC['property_stock']),
                    'goods_id'  => $goods_id
                );
               pdo_insert('eso_sale_goods_property_list', $data);
            }
            if($property_attr == 'del'){
                $property_id = $_GPC['property_id'];
                pdo_query("delete from " . tablename('eso_sale_goods_property_list') . " where uniacid={$_W['uniacid']} and id = $property_id");
            }
            $property1_list = pdo_fetchall("select * from " . tablename('eso_sale_goods_property1') . " where enabled= '1' order by displayorder asc");
            $property2_list = pdo_fetchall("select * from " . tablename('eso_sale_goods_property2') . " where enabled= '1' order by displayorder asc");
            $property_sql =  "SELECT li.*,pr1.name as property1_name, pr2.name as property2_name FROM " . tablename('eso_sale_goods_property_list')." li ";
            ...
            $property_sql .= " WHERE li.uniacid = '{$_W['uniacid']}' and li.goods_id ='".$goods_id."' ";
            $property_list  = pdo_fetchall($property_sql);
            ...
            if (checksubmit('submit')) {
                ...
                $data = array(
                    ...
                    'property1'=>$_GPC['property1'],
                    'property2'=>$_GPC['property2'],
                );

                if($data['freight'] == '-1'){
                    $freight = pdo_get('eso_sale_dispatch' , array('uniacid'=>$_W['uniacid']));
...
//            pdo_delete('eso_sale_goods', array('id' => $id));
    ...
    public function doWebOutOrder(){
        ...
        $info = pdo_fetchall(" SELECT * FROM " . tablename('eso_sale_order') . " WHERE uniacid = '{$_W['uniacid']}' $condition ");
        // var_dump("SELECT * FROM " . tablename('eso_sale_order') . " WHERE uniacid = '{$_W['uniacid']}' $condition ");exit;
// var_dump($info);exit;
...
// ./source/modules/public/Classes/PHPExcel.php
        ...
        $objPHPExcel->getProperties()->setCreator("订单列表")
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D1', '运费（元）')
            ->setCellValue('E1', '总价（元）')
            ->setCellValue('F1', '状态')
        ...
        foreach ($info as $i => $v) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A' . $i, $v['ordersn'])
                ->setCellValue('B' . $i, $address[$v['addressid']]['realname'])
                ->setCellValue('C' . $i, $address[$v['addressid']]['realname'])
                ->setCellValue('D' . $i, $v['dispatchprice'])
        ...
        // Rename worksheet
        $time = time();
        $objPHPExcel->getActiveSheet()->setTitle('订单列表' . $time);
...
//            }
    ...
    public function doMobilePhb()
    {
        ...
        // if($_W['uniacid'] == 7){
        //     $aa = "select sum(c.commission) as commission, m.realname, m.mobile from " . tablename('eso_sale_commission') . " as c left join " . tablename('eso_sale_member') . " as m on c.uniacid = m.uniacid and c.mid = m.id where c.flag = 0 and m.realname !='' and c.uniacid = " . $_W['uniacid'] . " and c.createtime >= " . $premonth . " and c.createtime <= " . $premonthed . " group by c.mid order by sum(c.commission) desc, c.createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        //     var_dump($aa);exit;
        // }
...
// 粉丝入口
    public function doMobileFansIndex()
    {
        ...
        if ($theone['promotertimes'] == 0 && $profile['flag'] == 0) {
            ...
            if (!$isorder) {
                message('您还未通过分销员审核，请等待管理员通过！', $this->mturl('list', array('mid' => $id)), 'success');
...
// var_dump($_W['siteroot'] . "app/" . $this->mturl('list', array('mid' => $id)));exit;
...
//-----------------------------------web端
    ...
    public function doWebCommission()
    {
        ...
        $members = pdo_fetchall("select id, realname, mobile, bankcard , banktype from " . tablename('eso_sale_member') . " where uniacid = " . $_W['uniacid'] . " and status = 1");
        $member = array();
        foreach ($members as $m) {
            $member['realname'][$m['id']] = $m['realname'];
            $member['mobile'][$m['id']] = $m['mobile'];
            $member['banktype'][$m['id']] = $m['banktype'];
            $member['bankcard'][$m['id']] = $m['bankcard'];
        }
        // 正在申请
        if ($op == 'display') {
            if ($_GPC['opp'] == 'check') {
    ...
    public function doWebOutCommission()
    {
        ...
        //var_dump($info);
        $commissionList = pdo_fetchall("SELECT c.*,m.realname,m.mobile,m.bankcard,m.alipay,m.wxhao FROM `ims_eso_sale_commission` AS c LEFT JOIN `ims_eso_sale_member` AS m ON c.mid=m.id WHERE c.createtime>=" . $starttime . " AND c.createtime<=" . $endtime . "  AND c.flag = 0 AND c.uniacid=" . $_W['uniacid'] . "  ");
        // AND c.isout = 0
        if($_W['uniacid'] == 7){   
            // echo 1;exit;
...
// ./source/modules/public/Classes/PHPExcel.php
	...
	//include $this->template('tuiguang');*/
        ...
        $description = $_W['account']['name'] . '分销系统，让分佣飞一会！';
        if(empty($gid)){
            $shareurl = $_W['siteroot'] . "app/" . $this->mturl('list', array('mid' => $id, 'id' => $gid));
        }else{
...
// var_dump($item['free_shipping']);exit;

        // $d['price'] = $this->getFreight($id);
        // if( $item['totalprice'] >= $item['free_shipping'] ){
        //     $d['price'] = 0;
        // }else{
            $d['price'] = $freight; 
        ...
        // $freight = pdo_get('eso_sale_freight' , array('uniacid'=>$_W['uniacid']));
        // if(empty($freight)){
        //     $freightdata = array('freight' => 10 , 'uniacid'=>$_W['uniacid']);
        //     pdo_insert('eso_sale_freight', $data);
        //     $freight['freight'] = 10;
        // }
    ...
    // // 查运费 
    // public function getFreight($goods_id ){
    //     $goods_freight = pdo_get('eso_sale_goods' , array('id'=>$goods_id) , array('freight','free_shipping','marketprice'));
    //     // if( $goods_freight['marketprice']*$buy_num >= $goods_freight['free_shipping'] ){
    //     //     $goods_freight['freight'] = 0;
    //     // }
    //     return $goods_freight['freight'];
    //     // if($)
    //     // $dispatch = pdo_fetch("select firstprice from " . tablename("eso_sale_dispatch") . " WHERE uniacid = {$_W['uniacid']}");
    //     // if(empty($dispatch)){
    //     //     $dispatch = 0;
    //     // }else{
    //     //     $dispatch = $dispatch['firstprice'];
    //     // }
    // }
    ...
    public function doMobileDetail()
    {
        ...
        // // 查运费
        // $dispatch = $this->getFreight($goodsid);
        // // 查包邮
        // $free_shipping = $this->getFreeShipping($goodsid);
    ...
    {
        global $_W, $_GPC;
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'freight';
        if ($operation == 'display') {
            ...
            // 运费配置
             if (checksubmit('submit')) {
                ...
                if(!is_numeric($data['firstprice'])){
                    message('请输入正确的运费金额！', $this->createWebUrl('dispatch', array('op' => 'freight')));
                }

                pdo_update('eso_sale_dispatch', $data, array('uniacid' => $_W['uniacid']));

                message('运费更新成功！', $this->createWebUrl('dispatch', array('op' => 'freight')), 'success');
            }


             
            $firstprice = pdo_fetch("SELECT * FROM " . tablename('eso_sale_dispatch') . " WHERE uniacid = '{$_W['uniacid']}'");
            if(empty($firstprice)){
                $data = array('firstprice' => 0 , 'uniacid'=>$_W['uniacid']);
                pdo_insert('eso_sale_dispatch', $data);
                $freight['firstprice'] = 10;
            }
...
// if($_W['uniacid'] == 7){
//     $a = "SELECT COUNT(*) FROM " . tablename('eso_sale_member') . " WHERE status = 1 and flag = 1 and (commission - zhifu) >= " . $zhifucommission . " AND `uniacid` = 7";
//     var_dump($a);exit;
// }
    ...
    private function getFromUser()
    {
        ...
        // if(empty( $_W['openid'] )){
        //     $callback = $_W['siteurl'];
        //     $forward = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$_W['uniaccount']['key']}&redirect_uri={$callback}&response_type=code&scope=snsapi_userinfo#wechat_redirect";
